...
- **検索ソースアーキテクチャ (ref.vim 風)**: 検索先を「ソース」として
  ...
  オプショナルソース (`lua/refsearch/sources/japanknowledge.lua`) となり、
  未使用時は読み込まれない。自前のソースは
  `lua/refsearch/sources/<name>.lua` として追加できる
...
- 履歴をソース名ごとに分けて保存 (旧フラット構造は japanknowledge の
  履歴として互換読み込み)
- 検索結果パネルの UI を `refsearch.picker` モジュールに分離し、
...
- `require("refsearch").word_under_cursor()`: カーソル下の検索語の抽出
  (辞書形化 + 複合名詞展開) を公開 API 化
- ソース追加のためのドキュメント (`:h refsearch-add-source`)
...
- `refsearch.word` モジュール: カーソル下の語の特定・複合名詞の展開・
...
- 機関固有の OpenAthens URL を既定値に含まないよう変更。
  `redirector` / `proxy` は `vim.g.refsearch_configuration` での設定が必須
- `vim.g.refsearch_configuration` が plugin 読み込み時に `config.setup()`
  に渡されていなかった問題
